...
# The mean loss gives an indication of the model's overall performance on the dataset.
...
# Create a list of characters corresponding to each embedding
chars = list(train_dataset.itos.values())

# Ensure we're using the smaller of the two lengths
n = min(len(chars), embeddings_3d.shape[0])
chars = chars[:n]
embeddings_3d = embeddings_3d[:n]
...
char_types = [get_char_type(c) for c in chars]

# Create the 3D scatter plot
fig = px.scatter_3d(x=embeddings_3d[:, 0], y=embeddings_3d[:, 1], z=embeddings_3d[:, 2],
                    color=char_types, text=chars,
                    hover_data={'character': chars, 'type': char_types})
...
fig.update_layout(scene=dict(xaxis_title='PCA 1',
                             yaxis_title='PCA 2',
                             zaxis_title='PCA 3'),
                  title='Character Embeddings Visualization')
...
print(f"Number of embeddings used: {n}")
print(f"Number of characters used: {len(chars)}")
